...
try:
    import websockets
    try:
        # websockets >= 13: 新位置（15.x 起旧路径 WebSocketClientProtocol 已废弃）
        from websockets.asyncio.client import ClientConnection as WSClientConnection
    except ImportError:
        # websockets 10~12 及更早：旧位置（这些版本上无废弃告警）
        from websockets.client import WebSocketClientProtocol as WSClientConnection
    # websockets >= 14 将 extra_headers 重命名为 additional_headers 并移除旧名
    WS_HEADERS_KW = 'additional_headers' if int(websockets.__version__.split('.')[0]) >= 14 else 'extra_headers'
    WEBSOCKETS_AVAILABLE = True
except ImportError:
    WSClientConnection = None  # type: ignore[assignment]
    websockets = None  # type: ignore[assignment]
    WS_HEADERS_KW = 'extra_headers'
...
class WebSocketEngine:
    ...
    async def connect(
        ...
        try:
            ...
            # websockets 15.x 已移除 extra_headers（改用 additional_headers）
            connect_kwargs: Dict[str, Any] = {WS_HEADERS_KW: extra_headers}
            if subprotocols:
                connect_kwargs['subprotocols'] = subprotocols
            if url.startswith('wss://') and ssl_context is not None:
