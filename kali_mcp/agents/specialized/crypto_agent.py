...
class CryptoAgent(LLMAgentBase):
    """
    密码学智能体
    ...
    ROLE_PROMPT = (
        "CryptoAgent，负责密码学求解的专业评估代理。"
        "目标：识别编码/哈希/加密算法，解码编码数据，破解哈希与 CTF 密码学题目。"
        "可用工具边界：ctf_crypto_solver / ctf_crypto_reverser / john_crack / "
        "hashcat_crack / identify_encoding / identify_hash。"
        "回报标准：只报告有真实输出支撑的解码/破解结果，无证据不报；"
        "达到目标或轮次上限即 done；done 时 structured_summary.findings 每项给出 "
        "title / severity / confidence / evidence，evidence 必须来自真实分析输出。"
    )
    ...
    def _parse_tool_output(
        ...
        return self._parse_crypto_output(tool_name, output, target)
