...
TOOL_REGISTRY: Dict[str, ToolSpec] = {
    ...
    "fastsec": ToolSpec(
        ...
        params=[
            ...
            ToolParam(name="concurrency", flag="-c", sanitize="arg", default=""),
            ToolParam(name="delay_min", flag="-delay-min", sanitize="arg", default=""),
            ToolParam(name="delay_max", flag="-delay-max", sanitize="arg", default=""),
            ToolParam(name="proxy", flag="-proxy", sanitize="arg", default=""),
            ToolParam(name="no_verify", flag="-no-verify", sanitize="arg", default="", join=""),
            # ---- 全模式参数（2026-08 字典+知识库集成后扩展） ----
            ToolParam(name="kb", flag="-kb", sanitize="arg", default=""),
            ToolParam(name="dir", flag="-dir", sanitize="arg", default=""),
            ToolParam(name="wordlist", flag="-w", sanitize="arg", default=""),
            ToolParam(name="brute", flag="-brute", sanitize="arg", default=""),
            ToolParam(name="service", flag="-service", sanitize="arg", default=""),
            ToolParam(name="port", flag="-port", sanitize="arg", default=""),
            ToolParam(name="user_file", flag="-U", sanitize="arg", default=""),
            ToolParam(name="pass_file", flag="-P", sanitize="arg", default=""),
            ToolParam(name="form_url", flag="-form-url", sanitize="arg", default=""),
            ToolParam(name="form_user", flag="-form-user", sanitize="arg", default=""),
            ToolParam(name="form_pass", flag="-form-pass", sanitize="arg", default=""),
            ...
            ToolParam(name="crack_wordlist", flag="-crack-wordlist", sanitize="arg", default=""),
            ToolParam(name="user", flag="-user", sanitize="arg", default=""),
            ToolParam(name="scan", flag="-scan", sanitize="arg", default=""),
            ToolParam(name="scan_range", flag="-scan-range", sanitize="arg", default=""),
            ToolParam(name="fingerprint", flag="-fingerprint", sanitize="arg", default=""),
            ToolParam(name="fingerprint_ports", flag="-fp-ports", sanitize="arg", default=""),
            ToolParam(name="cms", flag="-cms", sanitize="arg", default=""),
            ToolParam(name="audit", flag="-audit", sanitize="arg", default=""),
            ToolParam(name="file", flag="-file", sanitize="arg", default=""),
            ToolParam(name="shell", flag="-shell", sanitize="arg", default=""),
            ToolParam(name="s_host", flag="-s-host", sanitize="arg", default=""),
            ToolParam(name="s_port", flag="-s-port", sanitize="arg", default=""),
            ToolParam(name="s_enc", flag="-s-enc", sanitize="arg", default=""),
            ToolParam(name="listen", flag="-listen", sanitize="arg", default=""),
            ToolParam(name="seq", flag="-seq", sanitize="arg", default=""),
            ToolParam(name="dump", flag="-dump", sanitize="arg", default=""),
            ToolParam(name="dump_param", flag="-dump-param", sanitize="arg", default=""),
            ToolParam(name="sam", flag="-sam", sanitize="arg", default=""),
            ToolParam(name="system", flag="-system", sanitize="arg", default=""),
            ToolParam(name="smb", flag="-smb", sanitize="arg", default=""),
            ToolParam(name="smb_cmd", flag="-smb-cmd", sanitize="arg", default=""),
            ToolParam(name="smb_user", flag="-smb-user", sanitize="arg", default=""),
            ToolParam(name="smb_port", flag="-smb-port", sanitize="arg", default=""),
            ToolParam(name="smb_pass", flag="-smb-pass", sanitize="arg", default=""),
            ToolParam(name="kerberos", flag="-kerberos", sanitize="arg", default=""),
            ToolParam(name="domain", flag="-domain", sanitize="arg", default=""),
            ToolParam(name="kusers", flag="-kusers", sanitize="arg", default=""),
            ToolParam(name="kpass", flag="-kpass", sanitize="arg", default=""),
...
def _resolve_projectdiscovery_httpx() -> str:
    """Prefer ProjectDiscovery Go httpx over Python package named httpx."""
    candidates = []
    env_bin = (os.getenv("KALI_MCP_HTTPX_BIN") or "").strip()
    if env_bin:
        ...
        if os.path.isfile(path) and os.access(path, os.X_OK):
            try:
                if os.path.getsize(path) < 4096:
                    ...
                    if "from httpx import main" in head or "import httpx" in head:
                        continue
            except Exception:
                pass
            return path
...
def _build_httpx(data: Dict[str, Any]) -> str:
    """ProjectDiscovery httpx: prefer -u target; accept target/url/targets; close stdin."""
    raw = (
        data.get("target")
        or data.get("url")
        or data.get("targets")
    ...
    if isinstance(raw, (list, tuple)):
        target = str(next((x for x in raw if str(x).strip()), "")).strip()
    else:
        target = str(raw).strip().split()[0] if str(raw).strip() else ""
    if not target:
        logger.warning("httpx: missing target/url/targets")
        return ""
    target_q = sanitize_shell_arg(target)
    additional_args = (data.get("additional_args") or "").replace("-tech-detect", "-td")
    defaults = "-silent -status-code -title -server -timeout 5 -retries 0 -no-stdin"
    binary = _resolve_projectdiscovery_httpx()
    binary_q = sanitize_shell_arg(binary) if binary != "httpx" else "httpx"
    cmd = f"{binary_q} -u {target_q} {defaults}"
    if additional_args:
        cmd += f" {sanitize_shell_fragment(additional_args)}"
    return cmd
...
def build_command(tool_name: str, data: Dict[str, Any]) -> str:
    ...
    Resolution order:
        0. YAML recipe pilot when present and renderable.
    ...
    Returns:
        ...
        recognised.
    """
    # 0. Phase3 pilot: YAML recipes for high-frequency tools
    if not (isinstance(data, dict) and (data.get("no_recipe") or data.get("skip_recipe"))):
        try:
            from kali_mcp.core.recipe_loader import render_recipe_command

            recipe_cmd = render_recipe_command(
                tool_name, data if isinstance(data, dict) else {}
            )
            if recipe_cmd:
                return recipe_cmd
        except Exception as e:
            logger.debug("recipe render skipped tool=%s: %s", tool_name, e)
