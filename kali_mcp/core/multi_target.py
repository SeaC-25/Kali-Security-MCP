...
class MultiTargetOrchestrator:
    ...
    def orchestrate_attack(self, strategy: Optional[str] = None) -> Dict[str, Any]:
        """执行攻击编排"""
        if strategy:
    ...
    def _topological_sort(self, graph: Dict[str, List[str]]) -> List[List[str]]:
        ...
        语义：graph[target_id] = 该目标依赖的 target_id 列表（前置）。
        无依赖的目标（入度为 0）最先执行；被依赖者必须先于依赖者。
        """
        # 入度 = 前置依赖数量
        in_degree = {node: len(graph[node]) for node in graph}

        # 前置依赖集合（快速查找“依赖了 node 的节点”）
        dependents_of = {node: set() for node in graph}
        for node, deps in graph.items():
            for dep in deps:
                if dep in dependents_of:
                    dependents_of[dep].add(node)
