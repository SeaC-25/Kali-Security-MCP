...
SUPPORTED_PROFILES = {"harness", "strict", "compliance", "full"}
...
_HARNESS_ONLY_DISABLED = ALL_MODULE_KEYS - {"harness"}

DEFAULT_DISABLED_BY_PROFILE = {
    # Harness-only: main brain sees orchestration APIs only (recommended internal default).
    "harness": set(_HARNESS_ONLY_DISABLED),
    # Strict profile: assessment + harness lean surface.
    "strict": {
        "multi_agent",
        "recon",
        "ai_session",
...
def load_tool_profile(
    ...
    profile = (profile_name or os.getenv("KALI_MCP_TOOL_PROFILE", "harness")).strip().lower()
